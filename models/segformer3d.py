...
class MixVisionTransformer(nn.Module):
    def __init__(
        ...
        patch_embeddings, transformer_encoders, norms = (
            nn.ModuleList(),
            nn.ModuleList(),
            nn.ModuleList(),
        )
        ...
        self.transformer_encoders = transformer_encoders
        self.norms = norms
    ...
    def forward(self, x):
        ...
        for patch_embedding, transformer_encoder, norm in zip(
            self.patch_embeddings, self.transformer_encoders, self.norms
        ):
            x = patch_embedding(x)
            B, N, C = x.shape
            n = cube_root(N)
            for i, blk in enumerate(transformer_encoder):
                x = blk(x)
            x = norm(x)
...
class SegFormerDecoderHead(nn.Module):
    ...
    def __init__(
        ...
        self.linear_fuse = nn.Sequential(
            nn.Conv3d(
                in_channels=len(input_feature_dims) * decoder_head_embedding_dim,
        ...
        if scale_factor > 1:
            ...
            self.upsample_volume = nn.Identity()

    def forward(self, x):
        b, _, _, _, _ = x[0].shape
        
        feats = []
        for feat, linear in zip(x[1:], self.linears[1:]):
            feat = (
                linear(feat)
                .permute(0, 2, 1)
                .reshape(b, -1, feat.shape[2], feat.shape[3], feat.shape[4])
                .contiguous()
            )
        ...
        feats.append(
            self.linears[0](x[0])
            .permute(0, 2, 1)
            .reshape(b, -1, x[0].shape[2], x[0].shape[3], x[0].shape[4])
            .contiguous()
        )

        _c = self.linear_fuse(torch.cat(feats, dim=1))
        x = self.dropout(_c)
