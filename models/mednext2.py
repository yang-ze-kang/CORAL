...
class MedNeXt(nn.Module):
    ...
    def __init__(
        ...
        # self.stem = conv(in_channels, n_channels, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        if self.outside_block_checkpointing:
            ...
            x = checkpoint.checkpoint(
                self.down_0, x_res_0, use_reentrant=False
            )
            x_res_1 = self.iterative_checkpoint(self.enc_block_1, x)
            x = checkpoint.checkpoint(
                self.down_1, x_res_1, use_reentrant=False
            )
            x_res_2 = self.iterative_checkpoint(self.enc_block_2, x)
            x = checkpoint.checkpoint(
                self.down_2, x_res_2, use_reentrant=False
            )
            x_res_3 = self.iterative_checkpoint(self.enc_block_3, x)
            x = checkpoint.checkpoint(
                self.down_3, x_res_3, use_reentrant=False
            )

            x = self.iterative_checkpoint(self.bottleneck, x)
            if self.do_ds:
                x_ds_4 = checkpoint.checkpoint(
                    self.out_4, x, use_reentrant=False
                )

            x_up_3 = checkpoint.checkpoint(
                self.up_3, x, use_reentrant=False
            )
            dec_x = x_res_3 + x_up_3
            x = self.iterative_checkpoint(self.dec_block_3, dec_x)
            if self.do_ds:
                x_ds_3 = checkpoint.checkpoint(
                    self.out_3, x, use_reentrant=False
                )
            del x_res_3, x_up_3

            x_up_2 = checkpoint.checkpoint(
                self.up_2, x, use_reentrant=False
            )
            dec_x = x_res_2 + x_up_2
            x = self.iterative_checkpoint(self.dec_block_2, dec_x)
            if self.do_ds:
                x_ds_2 = checkpoint.checkpoint(
                    self.out_2, x, use_reentrant=False
                )
            del x_res_2, x_up_2

            x_up_1 = checkpoint.checkpoint(
                self.up_1, x, use_reentrant=False
            )
            dec_x = x_res_1 + x_up_1
            x = self.iterative_checkpoint(self.dec_block_1, dec_x)
            if self.do_ds:
                x_ds_1 = checkpoint.checkpoint(
                    self.out_1, x, use_reentrant=False
                )
            del x_res_1, x_up_1

            x_up_0 = checkpoint.checkpoint(
                self.up_0, x, use_reentrant=False
            )
            ...
            x = checkpoint.checkpoint(
                self.out_0, x, use_reentrant=False
            )
